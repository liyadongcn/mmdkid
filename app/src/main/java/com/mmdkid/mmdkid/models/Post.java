...
public class Post extends Model {
    protected final static String TAG = "Post";
    protected final static String URI = "v1/posts";
    protected final static String DEFAULT_WEB = "http://www.mmdkid.cn";

    public final static int TYPE_PRIVATE = 1; // 私贴
    public final static int TYPE_PUBLIC = 3;  // 公开贴
    public final static int STATUS_ACTIVE = 10; // 发布状态
    public final static int STATUS_DRAFT = 11;  // 草稿状态
    ...
    public static Post populateModel(JSONObject response) {
        ...
        try {
            ...
            if(response.has("updated_by")  && !response.isNull("updated_by")) model.updated_by = response.getInt("updated_by");
    ...
    public static ArrayList<Post> populateModels(JSONObject response){
        ...
        if(response.has("items")){
            ...
            try {
                ...
                for(int i = 0; i < currentPageTotal; i++){
                    Post model = populateModel(items.getJSONObject(i));
            ...
            Post model = populateModel(response);
            if(model!=null) arrayList.add(model);
        }
        return arrayList;
    }
    ...
    public JSONObject getJsonRequest(String action,Connection connection) {
        ...
        switch (action) {
            case Model.ACTION_CREATE:
                ...
                request.remove("id");
                Log.d(TAG,"Post json object is :" + request.toString());
                return  request;
            case Model.ACTION_UPDATE:
                ...
                if(id!=0){
                    ...
                    Log.d(TAG,"Post update url is " + connection.URL);
                }else {
                    Log.d(TAG,"Post for updating, but there is no post id.");
                    return null;
                }
                request = this.toJsonObject();
                request.remove("id");
            ...
            case Model.ACTION_DELETE:
                ...
                if(id!=0){
                    connection.URL = connection.URL + URI + "/"+ id;
                    Log.d(TAG,"Post delete url is " + connection.URL);
                }else {
                    Log.d(TAG,"Post for delete, but there is no post id.");
                    return null;
                }
                request = this.toJsonObject();
                request.remove("id");
                Log.d(TAG,"Post delete json object is :" + request.toString());
                return  request;
        }
        return null;
    }
    ...
    public static Query find(Context context, RESTAPIConnection.OnConnectionListener listener)
    {
        ...
        query.mModelClass = Post.class;
        return query;
    }

    @Override
    public String getUrl() {
        return DEFAULT_WEB+"/index.php?r=post/view&id="+ id +"&theme=app";
    }
}
