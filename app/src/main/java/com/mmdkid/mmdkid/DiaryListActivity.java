...
public class DiaryListActivity extends AppCompatActivity {
    ...
    private void initData() {
        ...
        mQuery = Diary.find(this, new RESTAPIConnection.OnConnectionListener() {
            @Override
                ...
                if(c == Diary.class && !responseDataList.isEmpty()){
                    ...
                    for(Object obj : responseDataList){
                        ...
                        if(!diary.mContent.equals("") && !diary.mContent.equals("null")){
                            diary.mContent=diary.mContent.replaceAll("<[.[^<]]*>","");
                            if(diary.mContent.length()>30) diary.mContent=diary.mContent.substring(0,30);
                            diary.setViewType(Model.VIEW_TYPE_DIARY_LIST);
                            mDataset.add(diary);
                        }
                    }
        ...
        }).where("user_id",Integer.toString(mKid.mId)).where("expand","image");
        mQuery.all();
    }
