...
public class VideoFragment extends Fragment {
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container,
        ...
        View fragmentView = inflater.inflate(R.layout.fragment_publish_manage_video, container, false);
        ...
        mRecyclerView.addOnItemTouchListener(new RecyclerViewClickListener(mContext, mRecyclerView, new RecyclerViewClickListener.OnItemClickListener() {
            ...
            public void onItemClick(View view, int position) {
                ...
                if (view.getId()==R.id.imageDelete){
                    showNormalDialog((Video) mDataset.get(position));
                }else{
            ...
            switch (view.getId()) {
                case R.id.delete:
                    if (mCurrentPosition != -1){
                        showNormalDialog((Video) mDataset.get(mCurrentPosition));
                    }
                    break;
            }
        }
    ...
    private void showNormalDialog(final Video video){
        /* @setIcon 设置对话框图标
         * @setTitle 设置对话框标题
        ...
        normalDialog.setPositiveButton("确定",
                new DialogInterface.OnClickListener() {
                    ...
                    public void onClick(final DialogInterface dialog, int which) {
                        video.delete(mContext, new RESTAPIConnection.OnConnectionListener() {
                            @Override
    ...
    private void initData() {
        ...
        if (!app.isGuest()) {
            ...
            mQuery = Video.find(mContext, new RESTAPIConnection.OnConnectionListener() {
                @Override
                public void onErrorRespose(Class c, String error) {
                ...
                public void onResponse(Class c, ArrayList responseDataList) {
                    ...
                    mIsFetching = false;
                    if(c == Video.class && !responseDataList.isEmpty()){
                        Log.d(TAG,"Get the content response from the server.");
                        for(Object obj : responseDataList){
                            Video video = (Video) obj;
                            Log.d(TAG,"Video id: " + video.id);
                            Log.d(TAG,"Video title: " +video.name);
                            Log.d(TAG,"Video images: " + video.imageList);
                            mDataset.add(video);
                            video.setViewType(Model.VIEW_TYPE_PUBLISH_MANAGE_VIDEO);
                        }
                        mAdapter.notifyDataSetChanged();
