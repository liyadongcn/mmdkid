...
public class PostFragment extends Fragment {
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container,
        ...
        View fragmentView = inflater.inflate(R.layout.fragment_publish_manage_post, container, false);
        ...
        mRecyclerView.addOnItemTouchListener(new RecyclerViewClickListener(mContext, mRecyclerView, new RecyclerViewClickListener.OnItemClickListener() {
            ...
            public void onItemClick(View view, int position) {
                ...
                if (view.getId()==R.id.imageDelete){
                    showNormalDialog((Post) mDataset.get(position));
                }else{
            ...
            switch (view.getId()) {
                case R.id.delete:
                    if (mCurrentPosition != -1){
                        showNormalDialog((Post) mDataset.get(mCurrentPosition));
                    }
                    break;
            }
        }
    ...
    private void showNormalDialog(final Post post){
        /* @setIcon 设置对话框图标
         * @setTitle 设置对话框标题
        ...
        normalDialog.setPositiveButton("确定",
                new DialogInterface.OnClickListener() {
                    ...
                    public void onClick(final DialogInterface dialog, int which) {
                        post.delete(mContext, new RESTAPIConnection.OnConnectionListener() {
                            @Override
    ...
    private void initData() {
        ...
        if (!app.isGuest()) {
            ...
            mQuery = Post.find(mContext, new RESTAPIConnection.OnConnectionListener() {
                @Override
                public void onErrorRespose(Class c, String error) {
                ...
                public void onResponse(Class c, ArrayList responseDataList) {
                    ...
                    if(c == Post.class && !responseDataList.isEmpty()){
                        Log.d(TAG,"Get the content response from the server.");
                        for(Object obj : responseDataList){
                            Post post = (Post) obj;
                            Log.d(TAG,"Post id: " + post.id);
                            Log.d(TAG,"Post title: " +post.title);
                            Log.d(TAG,"Post images: " + post.imageList);
                            mDataset.add(post);
                            post.setViewType(Model.VIEW_TYPE_PUBLISH_MANAGE_POST);
                        }
                        mAdapter.notifyDataSetChanged();
                    ...
                    .where("type", Integer.toString(Post.TYPE_PUBLIC))
                    .where("expand", "images");
            mQuery.all();
            mIsFetching = true;
        }
